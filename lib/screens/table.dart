...
class TablePage extends StatefulWidget{
  ...
  const TablePage({super.key, required this.firstname, required this.lastname, required this.age, required this.role, required this.email, required this.pass, required this.ethereumAddress});
...
class _TablePageState extends State<TablePage> {
  ...
  Future userDelete(String UID) async{ // asynchronous function built in the future and holds string uid in parameter
   try{ // try this code
  String uri = "http://localhost/flutter-login-signup/delete_requests.php";// holding the url of the php code that performed the delete query
          ...
          ElevatedButton(
              onPressed: () {
                ...
                Navigator.of(context)
                    ...
                    builder: (context) =>  TablePage(firstname: widget.firstname,lastname: widget.lastname,age: widget.age,role: widget.role,email: widget.role,pass: widget.pass,ethereumAddress: widget.ethereumAddress,)));
              },
              child: const Text('ok'))
        ],
      ),
  ...
  Future userAccept(String UID) async{//asynchronous function built in the future and holds string uid in parameter
    try{ // try this code
      String uri = "http://localhost/flutter-login-signup/accept_users.php";// holding the url of the php code that performed the accept query
      // in the uri string object
      var res = await http.post(Uri.parse(uri),body:{"UID":UID});// sending an http request to call the url and put it in res
      // parse method transforms the string uri to a valid uri
      var response = jsonDecode(res.body);// decoding the response after being encoded in the php file and get the result
  ...
  var data;
  String dataurl = "http://localhost/flutter-login-signup/table.php"; //PHP script URL
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: AppBar(
          title:const Text("Pending requests waitings for approval/Admin"), //title of app
