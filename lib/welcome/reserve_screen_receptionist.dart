...
import 'package:med/welcome/widget/my_appbar_receptionist.dart';
import 'package:med/welcome/widget/user_info.dart';

import 'package:med/widget/my_header.dart';

class ReserveScreen_rece extends StatelessWidget {
  ...
  const  ReserveScreen_rece({super.key, required this.firstname, required this.lastname, required this.age, required this.role, required this.email, required this.pass, required this.ethereumAddress});
        ...
        children: [// children of the column
          MyHeader(// calling my header class
            ...
            child: Column(// calling the child widget of the header
              children: [
                MyAppbar_rece(firstname: firstname,lastname: lastname,age: age,role: role,email: email,pass: pass,ethereumAddress: ethereumAddress),
                const SizedBox(// space between the appbar and userinfo
                  height: 10,// space with 10 px height
                ),
                const UserInfo(),// calling the userinfo class in order to get the doctor information
              ],
            ),
          ),
...
class ChooseSlot extends StatelessWidget {// chooseSlot class
  const ChooseSlot({// constractor
    Key? key,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {// build function of the choose slot class
    return Column(//column
      ...
      children: [// children of the column
        ...
        const SizedBox(// space between the text and row
          height: 14,// 14 px height
        ),
        Row(
          ...
          children: const [// children of the row
            ChooseDate(// calling the choose date class ,import 'package:med/welcome/widget/choose_date.dart'
              ...
              // if check is true means the date is valid
            ),
            ChooseDate(
              week: 'Sun',
              date: '21',
